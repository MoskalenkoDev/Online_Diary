...
const initialState =
{
  ...
  homework_teachers_li_list: [] // the list of teachers accepted request
};
...
export const homeworkReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case ACTION_DELETE_HOMEWORK_REQUESTS_TO_TEACHERS_LI_LIST_ITEM: {
      let newList = state.homework_requests_to_teachers_li_list.filter(obj => (obj.class_id !== action.payload)); // i hope it works correct
