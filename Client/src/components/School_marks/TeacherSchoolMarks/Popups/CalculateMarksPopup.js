...
export const AddMarksPopup = ({ lang, class_id, school_marks_popup_type, onHidePopup, school_subjects, showSuccessMessage}) => {
    ...
    const onCalculateMarks = async () => {
        if(!startDate || !endDate || !chosen_subject) {showWarning(); setPopup_warning_title(langObj[lang].warningTitleDateAndSubject)} 
        else {
            console.log("calculating student marks")
        }
    }
    ...
    const createCards = (img_src,name,surname,lastName, isBlocked = true, student_id = "", isStudentDeleted = false) => {
        
        return (
            <span> просто створюємо випадаючу лішку з інфою, на основі переданої інфи </span>
        )
    }
