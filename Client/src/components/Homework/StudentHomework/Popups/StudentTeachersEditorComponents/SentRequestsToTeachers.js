...
export const SentRequestsToTeachers = ({ state, lang }) => {
    ...
    let deleteTeacherFromRequestList = async (class_id) => {
        ...
        if (isDeleted) state.dispatch(ActionCreators.delete_homework_requests_to_teachers_li_list_item(class_id));
    }
    ...
    return (
        <div className={"homework_popup_menu_item_content_requests_to_teachers " + state.homework_popup_active_menu_item}>
            ...
            <ul className="homework_popup_menu_item_content_compact_cards_wrapper">
                ...
                <span className={"empty_list_message " + (state.homework_requests_to_teachers_li_list.length? "" : "visible")}>{langObj[lang].EmptyTeachersList}</span>
            </ul>
